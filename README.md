# Manga-Colorizer
This is a spin on https://github.com/BinitDOX/Manga-Colorizer, which I am working specifically on training for Boruto: Two-Blue Vortex. This repo is just a scratchpad, and as such should only be used for model training. Anyone interested in using the amazing model colorizer put together by BinitDOX please visit the above link and use his (probably more up-to-date) repository.

Just to re-iterate, this is only for Boruto TBV, with the goal of generating model weights and zipping them up into something like a GeneratorTBV.zip which can be swapped in and out as needed. I'd also like to document the general process for training the colorizer on a specific manga so that other communities may use it as desired.
